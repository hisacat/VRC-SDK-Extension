        ...
        private void CopyAvatar()
        {
            ...
            for (int avatarObjIndex = 0; avatarObjIndex < avatarObjCount; avatarObjIndex++)
            {
                ...
                for (int componentIndex = 0; componentIndex < componentsCount; componentIndex++)
                {
                    ...
                    //manage SkinnedMeshRenderer
                    if (component.GetType() == typeof(SkinnedMeshRenderer))
                    {
                        #region copy SkinnedMeshRenderer fields
                        var skin = component as SkinnedMeshRenderer;
                        var addedSkin = addedcomponent as SkinnedMeshRenderer;

                        addedSkin.quality = skin.quality;
                        addedSkin.updateWhenOffscreen = skin.updateWhenOffscreen;
                        addedSkin.skinnedMotionVectors = skin.skinnedMotionVectors;
                        addedSkin.sharedMesh = skin.sharedMesh;
                        if (skin.rootBone != null)
                        {
                            if (avatarTrfDic.ContainsKey(skin.rootBone))
                                addedSkin.rootBone = avatarTrfDic[skin.rootBone];
                        }
                        else
                            addedSkin.rootBone = null;
                        addedSkin.localBounds = skin.localBounds;
                        addedSkin.lightProbeUsage = skin.lightProbeUsage;
                        addedSkin.reflectionProbeUsage = skin.reflectionProbeUsage;
                        if (skin.probeAnchor != null)
                        {
                            if (avatarTrfDic.ContainsKey(skin.probeAnchor))
                                addedSkin.probeAnchor = avatarTrfDic[skin.probeAnchor];
                        }
                        else
                            addedSkin.probeAnchor = null;
                        addedSkin.shadowCastingMode = skin.shadowCastingMode;
                        addedSkin.receiveShadows = skin.receiveShadows;
                        addedSkin.motionVectorGenerationMode = skin.motionVectorGenerationMode;

                        //material copy only new added gamobject
                        if (newAddedComponents.Contains(addedSkin))
                            addedSkin.sharedMaterials = skin.sharedMaterials.Clone() as Material[];
                        #endregion
                    }
                    else
                    {
                        ...
                        for (int fieldIndex = 0; fieldIndex < fieldCount; fieldIndex++)
                        {
                            ...
                            var newField = addedcomponent.GetType().GetField(field.Name);
                        ...
                        for (int propertyIndex = 0; propertyIndex < propertiesCount; propertyIndex++)
                        {
                            ...
                            var newProperty = addedcomponent.GetType().GetProperty(property.Name);
                            ...
                            //Debug.Log(property.Name + " " + property.ReflectedType);
